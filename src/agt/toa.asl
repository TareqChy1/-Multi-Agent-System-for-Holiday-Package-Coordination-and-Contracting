...
max_price("Transport", 100).
max_price("Accommodation", 200).

min_rep("Transport", 50).
min_rep("Accommodation", 100).
...
// Used to find potential client's auction artifact
...
+!go <-
      ...
      makeArtifact(ora4mas, "ora4mas.nopl.OrgBoard", ["src/org/basic_org.xml"], OrgArtId)[wid(WOrg)];
      focus(OrgArtId);
      // create the group and adopt the role supervisor
      createGroup(basic_group, toa_group, GrArtId);
      ...
      !contract_winners("basic_group");

      // create the scheme
      createScheme(bhsch, basic_package, SchArtId);
      ...
      commitMission("complete_basic_package")[artifact_id(SchArtId)].
